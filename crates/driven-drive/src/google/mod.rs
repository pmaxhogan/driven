...
/// The exponential-backoff + classification middleware, re-exported from
/// [`driven_remote`] at its historical `driven_drive::google::retry` path. The
/// policy is backend-neutral, so it moved out of the Drive crate when the
/// second destination landed.
pub use driven_remote::retry;
...
// The `driven.*` `app_properties` vocabulary is declared ONCE in
// `driven_remote::props` (the executor STAMPS the keys, each backend SEARCHES
// on them, and a drifted second copy would make the audit query match nothing
// and re-upload the whole source). Re-exported here at the historical
// `driven_drive::google::*` paths.
pub use driven_remote::props::SOURCE_ID_KEY;
pub(crate) use driven_remote::props::{CLIENT_OP_UUID_KEY, FOLDER_MARKER_KEY};
...
// The classified SPEC s24 error taxonomy (`DriveError`, `classification_of`,
// `classify_stream_read_error`) is backend-neutral - the executor downcasts to
// it for pacer / circuit-breaker verdicts regardless of destination - so it
// lives in `driven_remote::error` beside the trait it is thrown across.
// Re-exported here at the historical `driven_drive::google::*` paths.
pub use driven_remote::error::{classification_of, classify_stream_read_error, DriveError};
