...
#[cfg(windows)]
pub fn volume_capacity(path: &Path) -> Option<(u64, u64)> {
    // Deliberately unimplemented rather than guessed: `about()` reports an
    // unknown capacity, which the UI renders as "unknown", and nothing in the
    // sync engine depends on it. A wrong number would be worse than none.
    let _ = path;
    None
}
