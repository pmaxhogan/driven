...
impl LocalFsConfig {
    ...
    pub fn normalized(mut self) -> Result<Self, LocalFsConfigError> {
        ...
        // equal, but never turn `/` into the empty string.
        let trimmed = {
            let t = root.trim_end_matches(std::path::MAIN_SEPARATOR);
            if t.is_empty() {
                root.as_str()
            } else {
                t
...
mod tests {
    ...
    fn normalization_trims_whitespace_and_a_trailing_separator() {
        let c = cfg("  /Volumes/Backup/  ").normalized().unwrap();
        assert_eq!(c.root, "/Volumes/Backup");
        // A bare root must survive.
        assert_eq!(cfg("/").normalized().unwrap().root, "/");
    }
    ...
    fn relative_and_empty_roots_are_rejected() {
        ...
        assert_eq!(
            cfg("backups").normalized().unwrap_err(),
        ...
        assert_eq!(
            cfg("/a\0b").normalized().unwrap_err(),
