...
- [ ] **Step 1: Write failing mount tests** - settings-pages.test.ts is a PORT, not a thin smoke file: move every test in settings-components.test.ts's "Settings Rules tab" describe (~44 tests, lines 1404-2925) into a describe for the page that now owns its control, re-pointing the mount from `Settings` to that page (fixtures copied across). Add one new test: SchedulePowerPage's schedule time input with invalid text (hhmmToMinutes -> null) commits NOTHING (the Task 1 deviation). The old "Settings Rules tab" describe is DELETED in this task once its tests all live here - this both keeps the coverage gate whole (the moved markup keeps its exercising tests) and clears the way for Task 3 to delete the `tab` prop without 44 stale mounts breaking.
- [ ] **Step 2: Run - expect FAIL.**
- [ ] **Step 3: Move each card block + its `set*` handlers + card-local state** out of Settings.vue into its page per the mapping table, splitting the Performance card's contents (scan interval -> GeneralPage, deep-verify -> AdvancedPage, VSS/APFS -> PlatformPage, the rest -> PerformancePage) and the Power-and-network card wholly into SchedulePowerPage together with the Schedule-window card. SchedulePowerPage's schedule commit guards `hhmmToMinutes`'s null (skip the commit, field unchanged) per the Task 1 deviation note. GeneralPage for now holds ONLY scan interval (channel arrives in Task 5). Keep all `t()` keys unchanged. Settings.vue's Rules branch becomes `<GeneralPage/><SchedulePowerPage/>...<AdvancedPage/>` stacked in order.
...
- Modify: `ui/src/router.ts`, `ui/src/views/Settings.vue`, `ui/src/App.vue`, `ui/src/locales/en-US.json`, `ui/src/__tests__/app-shell.test.ts`, `ui/src/__tests__/settings-components.test.ts` (ONLY the "Settings About tab" tab-strip-navigation tests - the two asserting the deleted tab `<nav>` buttons - rewritten as sidebar/route navigation; content assertions stay for Task 5), `ui/src/__tests__/views-b-polish.test.ts` (its one `mount(Settings, { props: { tab: "accounts" } })` at ~:164 re-pointed at the routed shell)
